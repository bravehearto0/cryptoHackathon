...
export default styled(InlineLoading)`
  ...
  @keyframes spinerAni {
    0% {
      top: ${p => sizeToPixel(p, 80)}px;
      left: ${p => sizeToPixel(p, 80)}px;
      width: 0;
      height: 0;
      opacity: 1;
    }
    100% {
      top: ${p => sizeToPixel(p, 10)}px;
      left: ${p => sizeToPixel(p, 10)}px;
      width: ${p => sizeToPixel(p, 140)}px;
      height: ${p => sizeToPixel(p, 140)}px;
      opacity: 0;
    }
  }
  @-webkit-keyframes spinerAni {
    0% {
      top: ${p => sizeToPixel(p, 80)}px;
      left: ${p => sizeToPixel(p, 80)}px;
      width: 0;
